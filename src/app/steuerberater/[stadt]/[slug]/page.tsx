...
export default async function ProfilePage({ params }: { params: { slug: string; stadt: string } }) {
  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="max-w-5xl mx-auto px-6 -mt-0 py-12">
        <div className={`bg-white rounded-xl border p-8 md:p-10 ${
          ...
          {/* Details: Öffnungszeiten, Sprachen, Berufserfahrung */}
          {(profile.openingHours || (profile.languages && profile.languages.length > 0) || profile.experienceYears) && (
              ...
              {profile.openingHours && (
                <div className="bg-forest-50 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <h3 className="text-xs font-semibold uppercase tracking-widest text-forest-400">Öffnungszeiten</h3>
                  </div>
                  <p className="text-sm text-forest-700">{profile.openingHours}</p>
                </div>
              )}
          ...
          {currentStadt?.lat && currentStadt?.lng && (
            <div className="mt-8 pt-8 border-t border-forest-100">
              <h2 className="text-xs font-semibold uppercase tracking-widest text-forest-400 mb-4">Standort</h2>
              <MapEmbed lat={currentStadt.lat} lng={currentStadt.lng} name={profile.name} />
            </div>
          )}
