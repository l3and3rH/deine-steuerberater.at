...
export default function SteuerberaterCard(props: Props) {
  ...
  // Service flags
  const services = [
    { label: "Gratis Erstgespräch", active: !!gratisErstgespraech, highlight: true },
    { label: "Online-Beratung", active: !!onlineBeratung, highlight: false },
    { label: "Beratung vor Ort", active: !!beratungVorOrt, highlight: false },
    { label: "Schnellantwort", active: !!schnellantwort, highlight: false },
    { label: "Abendtermine", active: !!abendtermine, highlight: false },
  ].filter((s) => s.active);
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-4 px-4 py-4 pl-6 sm:gap-5 sm:px-6 sm:py-5 sm:pl-7">
        {/* ── Avatar column ── */}
        <div className="flex-shrink-0 flex flex-col items-center gap-2 pt-0.5">
          {logo ? (
            ...
            <div
              className="w-16 h-16 sm:w-24 sm:h-24 rounded-xl flex items-center justify-center flex-shrink-0"
              style={{
                background: `linear-gradient(145deg, ${fromColor}, ${toColor})`,
              }}
            >
              <span className="font-display text-2xl sm:text-3xl font-semibold text-white/90 select-none">
                {name.charAt(0)}
              </span>
            </div>
          )}
        ...
        <div className="flex-1 min-w-0">
          ...
          {(adresse || telefon) && (
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mb-3">
              ...
              {telefon && (
                <a
                  href={`tel:${telefon}`}
                  className="hidden sm:flex items-center gap-1.5 text-xs text-forest-500 hover:text-forest-800 transition-colors"
                >
                  <svg className="w-3 h-3 flex-shrink-0 text-forest-400" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
          ...
          {beschreibung && (
            <div className="mb-3">
              <p className="text-sm text-forest-600 leading-relaxed line-clamp-2">
                {beschreibung}
              </p>
              <Link
                href={profileUrl}
                className="mt-1 inline-flex items-center gap-0.5 text-xs font-medium text-forest-400 hover:text-forest-700 transition-colors group"
              >
                Mehr lesen
                <svg
          ...
          {services.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-2.5">
              {services.map(({ label, highlight }) => (
                ...
                >
                  <svg className={`w-3 h-3 flex-shrink-0 ${highlight ? "text-emerald-500" : "text-forest-400"}`} fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
          ...
          {visibleTags.length > 0 && (
            <div className="mb-2.5">
              <p className="text-[10px] font-semibold text-forest-400 uppercase tracking-[0.1em] mb-1">
                Spezialisierungen
              </p>
          ...
          {mandantengruppen.length > 0 && (
            <div className="mb-2.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {mandantengruppen.slice(0, 4).map((m) => (
                  <span
                    key={m}
                    className="text-xs bg-blue-50 text-blue-700 border border-blue-100 px-2.5 py-0.5 rounded-md"
                  >
                    {m}
                  </span>
                ))}
          ...
          {/* Meta line: Ausbildung · Zulassung · Auszeichnung */}
          {(ausbildung || zulassungsjahr || auszeichnungen.length > 0) && (
          ...
          <div className="mt-3 pt-3 border-t border-forest-50">
            <div className="flex flex-wrap gap-2">
              {/* Primary: Anrufen – nur mobil */}
              {telefon && (
                <a
                  href={`tel:${telefon}`}
                  className="sm:hidden inline-flex items-center gap-1.5 bg-forest-900 text-cream-100 hover:bg-forest-700 px-4 py-2 rounded-lg font-semibold text-xs transition-colors justify-center"
                >
              ...
              {/* Primary: Anfrage senden → Profilseite Kontaktformular */}
              <Link
                href={`${profileUrl}#kontakt`}
                className="inline-flex items-center gap-1.5 bg-forest-900 text-cream-100 hover:bg-forest-700 px-4 py-2 rounded-lg font-semibold text-xs transition-colors justify-center"
              >
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              ...
              {/* Secondary: Profil ansehen */}
              <Link
                href={profileUrl}
                className="inline-flex items-center gap-1.5 border border-forest-200 text-forest-700 hover:bg-forest-50 hover:border-forest-300 px-4 py-2 rounded-lg font-semibold text-xs transition-colors justify-center"
              >
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              ...
              {/* Extras: Video & Bewertung (nur Premium) */}
              {isPremium && videoUrl && (
                <a
                  href={videoUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-xs text-forest-500 hover:text-forest-800 transition-colors font-medium px-2 py-2"
                >
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
